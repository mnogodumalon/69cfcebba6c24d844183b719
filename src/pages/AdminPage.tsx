...
const ENTITY_TABS = [
  ...
  { key: 'reparatur_&_wartung', label: 'Reparatur & Wartung', pascal: 'ReparaturWartung' },
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('produktverwaltung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'produktverwaltung': new Set(),
    'ersatzteilpreise': new Set(),
    'reparatur_&_wartung': new Set(),
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'reparatur_&_wartung': return (data as any).reparaturWartung as ReparaturWartung[] ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'reparatur_&_wartung' && fieldKey === 'produkt_referenz_rw') {
      const match = (lists.produktverwaltungList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.produkt_name ?? '—';
    }
    if (entity === 'reparatur_&_wartung' && fieldKey === 'verwendete_ersatzteile') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'reparatur_&_wartung': return REPARATURWARTUNG_FIELDS;
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'reparatur_&_wartung': return {
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'reparatur_&_wartung' || dialogState?.entity === 'reparatur_&_wartung') && (
        <ReparaturWartungDialog
          open={createEntity === 'reparatur_&_wartung' || dialogState?.entity === 'reparatur_&_wartung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'reparatur_&_wartung' ? handleUpdate : (fields: any) => handleCreate('reparatur_&_wartung', fields)}
          defaultValues={dialogState?.entity === 'reparatur_&_wartung' ? dialogState.record?.fields : undefined}
          produktverwaltungList={(data as any).produktverwaltung ?? []}
      ...
      {viewState?.entity === 'reparatur_&_wartung' && (
        <ReparaturWartungViewDialog
          open={viewState?.entity === 'reparatur_&_wartung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'reparatur_&_wartung', record: r }); }}
          produktverwaltungList={(data as any).produktverwaltung ?? []}
          ersatzteilpreiseList={(data as any).ersatzteilpreise ?? []}
        />
      )}
